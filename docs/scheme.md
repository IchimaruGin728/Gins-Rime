...
Gins-Rime 不是从零写一套 RIME 方案，而是在核心引擎的基础上做最小化叠加。

## 为什么选核心

核心引擎内置了 24 个 Lua 脚本，覆盖了大部分进阶需求：
...
这些不需要额外 patch，核心 schema 已内置，直接继承就能用。

## 继承方式

`gins.schema.yaml` 用 `__include` 完整继承核心配置，只覆盖三项：
...
核心使用 Unicode 带调拼音（`ā á ǎ à`），所有外挂词库必须使用相同格式，否则无法被声调辅助筛选识别。自建词库（zhwiki、gins-shici）均通过 Rust 的 `pinyin` crate 生成带调拼音。
