...
local function replace_schema(file_path, target_schema)
    ...
    -- 根据文件名决定替换模式
    if file_path:find("core_reverse") then
        content = content:gsub("([%s]*__include:%s*core_algebra:/reverse/)%S+", "%1" .. target_schema)
    elseif file_path:find("core_mixedcode") then
        content = content:gsub("([%s]*__patch:%s*core_algebra:/mixed/)%S+", "%1" .. target_schema)
    elseif file_path:find("core_english") then
        content = content:gsub("([%s]*__patch:%s*core_algebra:/english/)%S+", "%1" .. target_schema)
    elseif file_path:find("core%.custom") or file_path:find("core_pro%.custom") then
        content = content:gsub("([%s%-]*core_algebra:/pro/)%S+",  "%1" .. target_schema, 1)
        content = content:gsub("([%s%-]*core_algebra:/base/)%S+", "%1" .. target_schema, 1)
    end
...
local function translator(input, seg, env)
    ...
    if input == "/zjf" or input == "/jjf" then
        ...
        local paths = {
            user_dir .. "/core_pro.custom.yaml",
            user_dir .. "/core.custom.yaml",
        }
        ...
        for _, p in ipairs(paths) do
            if file_exists(p) then
                ...
                local n1, n2 = 0, 0
                content, n1 = content:gsub("(%-+%s*core_algebra:/pro/)直接辅助(%s*#?.*)", "%1" .. target_aux .. "%2")
                content, n2 = content:gsub("(%-+%s*core_algebra:/pro/)间接辅助(%s*#?.*)", "%1" .. target_aux .. "%2")
                local n = n1 + n2
    ...
    if target_schema then
        ...
        -- 检查根目录是否存在自定义文件
        local pro_file = user_dir .. "/core_pro.custom.yaml"
        local normal_file = user_dir .. "/core.custom.yaml"
        local custom_file_exists = file_exists(pro_file) or file_exists(normal_file)

        local files = {
            "core_mixedcode.custom.yaml",
            "core_reverse.custom.yaml",
            "core_english.custom.yaml"
        }

        -- 判断是否为专业版
        local is_pro = core.is_pro_scheme(env)
        local fourth_file = is_pro and "core_pro.custom.yaml" or "core.custom.yaml"
