...
local function is_text_in_charset(env, ctx, text)
    ...
    for _, codepoint in utf8_codes(text) do
        ...
        if core.IsChineseCharacter(character) then
...
function M.init(env)
    ...
    if dist == "weasel" then
        ...
        charsetFile = core.get_filename_with_fallback("lua/data/charset.reverse.bin") or "lua/data/charset.reverse.bin"
    end
...
function M.func(input, env)
    ...
    if core and core.s2t_conversion then
        is_functional = core.s2t_conversion(ctx)
    end
