--@amzxyz https://github.com/amzxyz/rime_core
--core_lookup: #设置归属于super_lookup.lua
  --tags: [ abc ]  # 检索当前tag的候选
  --key: "`"       # 输入中反查引导符
  --lookup: [ core_reverse ] #反查滤镜数据库
...
local function get_schema_rules(env)
    ...
    local db_list = config:get_list("core_lookup/lookup")
...
function f.init(env)
    ...
    -- 1. 开启声调
    env.enable_tone = config:get_bool('core_lookup/enable_tone')
    if env.enable_tone == nil then env.enable_tone = true end

    -- 2. 读取数据源
    local sources_list = config:get_list('core_lookup/data_source')
    env.data_sources = {}
        ...
        for _, db_obj in ipairs(env.db_table) do
            ...
            if res_str and #res_str > 0 then
                for word in res_str:gmatch("%S+") do
                    local cand = Candidate("core_shadow", s_end, #ctx_input, word, "")
                    cand.quality = 1 
                    yield(cand)
                end
            end
