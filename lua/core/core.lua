...
---@author amzxyz
---@return boolean
function core.is_mobile_device()
...
---@param env Env
---@return boolean
function core.is_pro_scheme(env)
    -- local schema_name = env.engine.schema.schema_name
    -- return schema_name:gsub("PRO$", "") ~= schema_name
    return env.engine.schema.schema_id == "core_pro"
end

-- 以 `tag` 方式检测是否处于反查模式
function core.is_in_radical_mode(env)
    local seg = env.engine.context.composition:back()
    return seg and (
        seg:has_tag("core_reverse")
    ) or false
end
...
---@param context Context | nil
---@return boolean
function core.s2t_conversion(context)
    if not context or not context.composition or context.composition:empty() then
        return false
    end
...
-- 判断字符是否为汉字
function core.IsChineseCharacter(text)
...
function core.load_file_with_fallback(filename, mode)
    mode = mode or "r" -- 默认读取模式

    local _filename = core.get_filename_with_fallback(filename)
...
function core.get_input_method_type(env)
    ...
    for i = 0, n - 1 do
        ...
        if s then
            ...
            for symbol, id in pairs(core.INPUT_METHOD_MARKERS) do
...
--- 调用加载函数
function core.load_regex_patterns(config, path)
    local patterns = {}
