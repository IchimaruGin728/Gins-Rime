...
-- @amzxyz
-- https://github.com/amzxyz/rime_core
...
function M.init(env)
    ...
    env.sc_last_key = nil
    env.is_t9 = false
    if core.get_input_method_type then
        local im_type = core.get_input_method_type(env)
        if im_type == "t9" then
            env.is_t9 = true
        end
    end
    ...
    env.kp_func_patterns = core.load_regex_patterns(config, "recognizer/patterns")
    ...
    -- [ToneFallback] 声调容错
    env.tone_state = "idle"
    env.lookup_key = config:get_string('core_lookup/key') or '`'
...
local function handle_backspace(key, env, ctx)
    ...
    if env.bs_sequence then
        if not core.is_mobile_device() then
...
local function handle_number_logic(key, env, ctx)
    ...
    if kp_num ~= nil and not core.is_mobile_device() then
    ...
    if r:match("^[0-9]$") then
        digit_str = r
    elseif kp_num ~= nil and core.is_mobile_device() then
    ...
    if digit_str then
        ...
        if env.enable_tone_fallback then
            ...
            if core.is_function_mode_active then
                is_func_mode = core.is_function_mode_active(ctx)
            end
