...
function F.init(env)
    ...
    local delimiter_str = " '" 
    if cfg then
        local str = cfg:get_string("core_english/english_spacing")
        if str then env.english_spacing_mode = str end
        local timeout = cfg:get_double("core_english/spacing_timeout")
        if timeout then env.spacing_timeout = timeout end
        local key = cfg:get_string("core_lookup/key")
        if key and key ~= "" then env.lookup_key = key end
        local max_cands = cfg:get_int("core_english/max_candidates")
        if max_cands then env.max_eng_cands = max_cands end
        local sym = cfg:get_string("core_english/trigger")
...
function F.func(input, env)
    ...
    for cand in input:iter() do
        ...
        if not is_garbage then
            ...
            if skip_cand then
                ...
                if env.schema_id == "core_english" and fmt_cand.comment and find(fmt_cand.comment, "\226\152\175") then
    ...
    if not has_valid_candidate then
        ...
        if env.schema_id == "core_english" and has_letters(curr_input) then
            local anchor = nil
            local diff = ""
