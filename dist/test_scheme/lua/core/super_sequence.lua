...
function seq_data._ensure_export_file()
    ...
    if not _file_exists(export_path) then
        ...
        local user_id = core.get_user_id()
...
function P.func(key_event, env)
    ...
    if code == 0xffe3 or code == 0xffe4 then
        if context.composition:empty() then return core.RIME_PROCESS_RESULTS.kNoop end 
        ...
        return core.RIME_PROCESS_RESULTS.kNoop 
    end
    ...
    local function get_adjust_code()
        if core.is_function_mode_active(context) then
    ...
    if (not core.is_function_mode_active(context)) and _is_single_lowercase_letter(adjust_code) then
        return core.RIME_PROCESS_RESULTS.kNoop
    end
    ...
    process_adjustment(context)
    return core.RIME_PROCESS_RESULTS.kAccepted
end
------------------------------------------------------------
-- 十、Filter (含标记可视化)
------------------------------------------------------------
...
function F.func(input, env)
    ...
    local adjustment_allowed = not (core.is_function_mode_active(context) and seq_property.get(context) == nil)
    ...
    local cands, seen = {}, {}
    local is_fun_mode = core.is_function_mode_active(context)
