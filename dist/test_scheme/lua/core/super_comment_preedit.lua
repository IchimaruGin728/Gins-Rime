...
function CR.init(env)
    ...
    local file, close_file, err = core.load_file_with_fallback(path)
    if not file then
...
function ZH.func(input, env)
    ...
    local is_radical_mode = core.is_in_radical_mode(env)
    local schema_id = env.engine.schema.schema_id or ""
    local is_core_pro = (schema_id == "core_pro")
    local should_skip_candidate_comment = core.is_function_mode_active(context) or input_str == ""
