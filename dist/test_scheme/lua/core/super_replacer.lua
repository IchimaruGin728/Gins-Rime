...
local userdb = safe_require("core/userdb")
local core = safe_require("core/core")
...
local function connect_db(db_name, current_version, delimiter, tasks, config_sig)
    ...
    if db:open_read_only() then
        local db_ver = db:meta_fetch("_core_ver") or ""
    ...
    if needs_rebuild then
        if db:open() then
            ...
            fmm_cache = {} 
            db:meta_update("_core_ver", current_version)
...
function M.init(env)
    ...
    if core and core.version then
        current_version = core.version
    end
    env.input_type = "unknown"
    if core and core.get_input_method_type then
        env.input_type = core.get_input_method_type(env)
    end
