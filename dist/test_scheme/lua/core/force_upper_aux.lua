...
function ForceUpperAux.init(env)
    ...
    env.aux_cache = {}
    env.dict = ReverseLookup(config:get_string("translator/dictionary") or "core_pro")
    ...
    env.on_update = function(ctx)
        ...
        -- 遇到转换模式或功能面板时立刻放行
        local is_special_mode = core.s2t_conversion and core.s2t_conversion(ctx)
        if env.is_cycling or core.is_function_mode_active(ctx) or is_special_mode then 
            return 
        end
...
function ForceUpperAux.func(key_event, env)
    ...
    -- 拦截转换状态
    local is_special_mode = core.s2t_conversion and core.s2t_conversion(ctx)
    if core.is_function_mode_active(ctx) or is_special_mode then 
        return 2 
    end
